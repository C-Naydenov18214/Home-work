...
--data Maybee a = Nothing_Nothing | Just_Just a
 data Lol = Lol | Kek| Cheburek deriving Show 
...
--  Возвращает пустую хэштаблицу
 ...
 hashFunc [] 0 = 0
 hashFunc xs len = foldr (\x sum -> (ord x)*len*abs ((ord x) - len) + sum) 0  xs      -- (ord x)*(len `mod` (ord x)) `mod` (abs(len - ord x)) + (hashFunc xs (len-1))
